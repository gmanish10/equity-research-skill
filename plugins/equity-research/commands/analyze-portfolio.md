...
- **ADD** — tickers/ETFs with sizing, role, thesis. **Always run an idea-generation pass** (SKILL.md → Section 5), not only when a gap is flagged: `screen_stocks` + sector/industry data → filter vs. holdings → subagent mini deep-dive → surface the **top 2–3 new candidates that would most improve the book**. Fill a gap if there is one, else propose the best risk-adjusted upgrades as optional adds. If nothing clears the quality bar, say "screened, nothing compelling." Never pull new names from memory; prefer ETF equivalents over mutual funds for new money.
